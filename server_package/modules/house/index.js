const { CreateDynamicPickup, CreateDynamic3DTextLabel, DestroyDynamicPickup, DestroyDynamic3DTextLabel, UpdateDynamic3DTextLabelText } = require("../../libs/streamer");
    ...
    async Create(id, owner, position, interiorType, cost, lifts) {
        ...
        for(let i = 0; i < lifts.length; i++) {
            ...
            for(let labelLoop = 0; labelLoop < lifts[i].positionsZ.length; labelLoop++) {
                this.Info[index].lifts[liftIndex].labels.push(CreateDynamic3DTextLabel(`/lift\n{BBFF00}Level: {FF0000}${(labelLoop)}`, -1, lifts[i].positionXYZ[0], lifts[i].positionXYZ[1], lifts[i].positionXYZ[2] + lifts[i].positionsZ[labelLoop], 5));
            }
        }
    },
    async Update(id) {
