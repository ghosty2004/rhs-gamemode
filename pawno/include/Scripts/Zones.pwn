...
stock GetPlayer3DZoneName(playerid)
{
    new string[123];
    new Float:p[3];
    ...
    for(new i = 0; i != sizeof(gSAZones); i++)
    {
        if(p[0] >= gSAZones[i][SAZONE_AREA][0] && p[0] <= gSAZones[i][SAZONE_AREA][3] && p[1] >= gSAZones[i][SAZONE_AREA][1] && p[1] <= gSAZones[i][SAZONE_AREA][4] && p[2] >= gSAZones[i][SAZONE_AREA][2] && p[2] <= gSAZones[i][SAZONE_AREA][5])
        {
 	    	format(string, sizeof(string), "%s", gSAZones[i][SAZONE_NAME]);
        }
    }
	return string;
}
